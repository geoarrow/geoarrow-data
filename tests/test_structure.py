...
def read_format_arrows(file: model.File):
    with ipc.open_stream(file.path) as reader:
        ...
        check_field_names(table.column_names)
...
def check_field_names(field_names):
    all_names = list(field_names)

    assert "geometry" in field_names
    assert "geometry" == field_names[-1]

    unique_names = set(all_names)
    if len(all_names) != len(unique_names):
        lab = ", ".join("{v}" for v in all_names)
        raise ValueError(f"Non-unique names: {lab}")

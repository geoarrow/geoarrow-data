...
for f in glob.glob('example/*-wkt.parquet'):
    ...
    schema_out = pa.schema([pa.field('geometry', type_out)]) \
        .with_metadata({'geo': geo_meta_out})

    geom_out = pa.chunked_array(
        [array.as_geoarrow(type_out) for array in geom.chunks]
    )

    table_out = pa.table([geom_out], schema=schema_out)
    parquet.write_table(
        table_out,
        re.sub('-wkt.parquet', '.parquet', f),
    ...
    parquet.write_table(
        table_out,
        re.sub('-wkt.parquet', '-interleaved.parquet', f),
        compression='uncompressed'
    )

...
zip_codes_wkb = pa.table(
    ...
    schema=pa.schema(
        [pa.field("zip_code", pa.utf8()), pa.field("geometry", geom_type)]
    ).with_metadata({"geo": geo_meta_json}),
)

parquet.write_table(
    zip_codes_wkb, "us-zip-codes/us-zip-codes-wkb.parquet", compression="ZSTD"
)

geom_type_interleaved = (
    ga.multipolygon()
...
zip_codes_interleaved = pa.table(
    ...
    schema=pa.schema(
        [pa.field("zip_code", pa.utf8()), pa.field("geometry", geom_type_interleaved)]
    ).with_metadata({"geo": geo_meta_json}),
)
...
geom_type_geoarrow = ga.multipolygon().with_crs(crs84_json, ga.CrsType.PROJJSON)
...
zip_codes_geoarrow = pa.table(
    ...
    schema=pa.schema(
        [pa.field("zip_code", pa.utf8()), pa.field("geometry", geom_type_geoarrow)]
    ).with_metadata({"geo": geo_meta_json}),
)
